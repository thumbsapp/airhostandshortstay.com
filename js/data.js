...
// data.js – enhanced with incentives (non-destructive addition)
MockData.properties.forEach(prop => {
    if (!prop.incentives) {
        // assign some default incentives based on tier or random
        if (prop.tier === 'diamond') {
            prop.incentives = [
                { type: 'cashback', category: 'financial', value: 15, description: '15% cashback' },
                { type: 'spa_credit', category: 'experience', value: 30, description: '$30 spa credit' },
                { type: 'late_checkout', category: 'experience', value: 20, description: 'Free late checkout' }
            ];
        } else if (prop.tier === 'gold') {
            prop.incentives = [
                { type: 'cashback', category: 'financial', value: 10, description: '10% cashback' },
                { type: 'breakfast', category: 'physical', value: 15, description: 'Free breakfast daily' }
            ];
        } else {
            prop.incentives = [
                { type: 'welcome_drink', category: 'experience', value: 10, description: 'Welcome drink on arrival' }
            ];
        }
    }
});
